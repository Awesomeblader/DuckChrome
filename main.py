...
# Everything run in pygame is a loop, basically arduino
HEIGHT = 450
WIDTH = 1100
...
class Goose:
    ...
    def update(self, Input):
        ...
        if Input[pygame.K_UP] and not self.goose_jump:
            self.goose_crouch = False
            self.goose_run = False
            self.goose_jump = True
        elif Input[pygame.K_DOWN] and not self.goose_jump:
            self.goose_crouch = True
            self.goose_run = False
            self.goose_jump = False
        elif not (self.goose_jump or Input[pygame.K_DOWN]):
            self.goose_crouch = False
            self.goose_run = True
            self.goose_jump = False
    ...
    def jump(self):
        ...
        if self.goose_jump:
            # RECALL, y axis is positive DOWNWARDS
